...
The skills under `skills/` are taken from [obra/superpowers](https://github.com/obra/superpowers) v6.4.2 at commit [`8ca22db`](https://github.com/obra/superpowers/commit/8ca22dba9a94f28898bbce59f2537ff4d87c747d); the exact upstream version, commit and repository are recorded in `package.json`'s `superpowers` field. **Two deviations from upstream, neither changing meaning**: (1) formatting — `brainstorming/scripts/helper.js`, `brainstorming/scripts/server.cjs`, `systematic-debugging/condition-based-waiting-example.ts` and `writing-skills/render-graphs.js` were reflowed by this repository's prettier (upstream did not touch them in 6.4.x); (2) invocation — four script calls in `executing-plans/SKILL.md` gained a `bash` prefix (`bash scripts/task-start …` and friends). Deviation (2) covers an upstream miss: those scripts are 100644 in git with no executable bit, so a bare call fails with `Permission denied` — the very bug upstream fixed in 6.4.1 for `subagent-driven-development` but not in the `executing-plans` skill it wrote in the same release. Every other file is byte-for-byte upstream. The optional visual companion in `brainstorming` loads an upstream-hosted logo containing the Superpowers version; it sends no project or prompt content. Set `SUPERPOWERS_DISABLE_TELEMETRY` to a true value to disable it.
