...
1. **DSH**：`^0.1.7-rc.2`（声明在 `engines.dsh` 与 `peerDependencies`；见「兼容性」）。
...
> `peerDependencies` 与 `engines.dsh` 写的是 `^0.1.7-rc.2`。semver 的规则是：带 prerelease
> 标签的版本，只能被**元组相同**的比较器接纳，所以这条 caret 收下 `0.1.7-rc.2` 与同元组的
> 后续 prerelease（实测 semver 7.8.5：`0.1.7-rc.2` / `0.1.7-rc.3` / `0.1.7` / `0.1.8`
> 满足，`0.2.0-rc.1` / `0.2.0` 不满足）；`0.1.6-alpha.*` 与 `0.1.5-*` 已不在声明范围内
> （上方表里列出的是**校对过**的版本，不等于仍被 peer 范围接纳——实测 `0.1.6-alpha.2`
> 与 `0.1.6` 都不满足这条 caret）。写 `-0` 后缀（如 `>=0.1.7-rc.2-0`）是**错的**，实测它
> 连 `0.1.7-rc.2` 本身都不接纳（`0.1.7` 才满足）。
