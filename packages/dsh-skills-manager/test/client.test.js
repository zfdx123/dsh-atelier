...
function fakePrimitives() {
  ...
  // `Button` 在外壳里是 `React.forwardRef(...)` 的**对象**（`$$typeof` + `render`），
  // 不是普通函数。以前替身写成函数，于是「守门用 typeof primitives.Button ===
  // 'function'」这个 bug 一路绿灯：真实运行时 typeof 恒为 'object'，整套原语被判
  // 不可用，走 Button 的主按钮与图标位全部退回字形。替身必须长得跟真货一样。
  const forwardRef = (name) => {
    const render = (props) => ({ type: `ui:${name}`, props: props ?? {}, children: props?.children ?? [] })
    render.primitiveName = name
    return { $$typeof: Symbol.for('react.forward_ref'), render, primitiveName: name }
  }
  return {
    Modal: make('Modal'),
    ...
    Toast: make('Toast'),
    IconCheckOutlineRegular: make('IconCheckOutlineRegular'),
    IconWarningOutlineRegular: make('IconWarningOutlineRegular'),
    IconCheckOutlineRegular: make('IconCheckOutlineRegular'),
    IconPlusOutlineRegular: make('IconPlusOutlineRegular'),
    IconSkillOutlineRegular: make('IconSkillOutlineRegular'),
  }
}
...
test('ToastHost 渲染原生 Toast（成功用对勾图标），没有提示时渲染 null', async () => {
  ...
  assert.equal(host.props.icon.type, ui.IconCheckOutlineRegular)
...
test('图标（有原语）：新建按钮走 Button 的 icon 通道，文案不再自带「＋」', async () => {
  ...
  assert.equal(add[0].props.icon.type, ui.IconPlusOutlineRegular)
  assert.equal(add[0].children[0], '新建技能', '有真图标时文案里不再重复一个「＋」')
})

test('图标（降级）：拿不到原语时新建按钮仍是「＋ 新建技能」，icon() 报 null 不抛错', async () => {
  const { exports } = await loadClient({ primitives: false })
  assert.equal(exports.icon('IconPlusOutlineRegular', 16), null, '没有图标库时图标位必须是 null')
  const plain = findAll(
...
test('图标（有原语）：诊断行用 IconWarningOutlineRegular，不再拼「✖ / !」', async () => {
  const { exports, ui } = await loadClient()
  const row = exports.SkillRow(Object.assign({}, ROW_ARGS, { skill: SKILL_WITH_ISSUES }))
  assert.equal(findAll(row, (node) => node.type === ui.IconWarningOutlineRegular).length, 2, '两条诊断各带一个图标')
...
test('图标（降级）：拿不到原语时诊断行仍拼「✖ / !」，icon() 报 null 不抛错', async () => {
  const { exports } = await loadClient({ primitives: false })
  assert.equal(exports.icon('IconWarningOutlineRegular', 14), null)
...
test('图标（有原语）：弹窗里选中的根目录用 IconCheckOutlineRegular，文案不带「✓ 」', async () => {
  const { exports, ui } = await loadClient()
  const host = exports.DialogHost({
    dialog: { kind: 'move', target: SKILL },
  ...
  assert.equal(rows[0].props.icon.type, ui.IconCheckOutlineRegular, '默认选中的第一行用图标代替「✓ 」')
...
test('图标（降级）：拿不到原语时根目录行退回自带按钮 +「✓ 」字形', async () => {
  const { exports } = await loadClient({ primitives: false })
  assert.equal(exports.icon('IconCheckOutlineRegular', 16), null)
...
test('图标（有原语）：侧栏入口用 IconSkillOutlineRegular，不再渲染「◈」', async () => {
  const { exports, ui } = await loadClient()
  const entry = exports.SidebarEntry({ ctx: {} })
  assert.equal(findAll(entry, (node) => node.type === ui.IconSkillOutlineRegular).length, 1)
  assert.equal(findAll(entry, (node) => node.type === 'span' && node.children[0] === '◈').length, 0)
})

test('图标（降级）：拿不到原语时侧栏入口仍渲染「◈」字形', async () => {
  const { exports } = await loadClient({ primitives: false })
  assert.equal(exports.icon('IconSkillOutlineRegular', 14), null)
  const entry = exports.SidebarEntry({ ctx: {} })
