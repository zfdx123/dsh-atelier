...
//
// 而管理器面板里「添加技能文件夹」写进的是**本插件的设置条目**。那个条目的名字是挂载
// 这一行的 loader 条目 id（入口包插的是 `dsh-skills-manager`，单装时是
// `skill-manager`，放进 include 分组还会带组前缀），所以这里不能写死一个名字——插件
// 按值形状认领它（见 index.js 的 claimEntry）。两者毫无关系。于是出现最坏的一种情况：面板里明明列出 107 个技能，DSH 却一个都不认识、
