...
MCP 服务器管理器：给 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（DSH）加一个「设置 → MCP 服务器」页面，在界面上配置 MCP（Model Context Protocol）服务器，**保存即生效**——宿主侧自动挂载/卸载 `@deepseek-ai/dsh-mcp-client` 实例，我（模型）随即拿到或失去 `mcp__<名称>__<工具名>` 形式的工具。
...
- 每台服务器可配置 `toolCallTimeoutMs`（工具调用超时）、`failOnStartupError`（启动失败即报错），以及断线重连（`reconnectEnabled` / `reconnectMaxAttempts`，对应 mcp-client 0.1.5 的 `reconnect` 配置）
...
- **HTTP 接口不裸奔**：优先注册在 DSH 0.1.5 的 `/api` 共享通道精确 Fetch 路由上（宿主施加 Host/Origin 信任栅栏 + 浏览器鉴权），拿不到 `connection` 服务时才退化为裸 `webServer` 路由并自行施加同样的判定（退到只允许回环来源）
...
本仓库以本地工作区方式装进 profile（`dsh plugin` 会把它写进 profile 依赖并应用 bundle 层）：

```sh
dsh plugin --profile web add E:\work\ai\dsh-mcp-manager
```
...
卸载：`dsh plugin --profile web remove @zfdx123/dsh-mcp-manager`（同时移除依赖与 bundle 层）。

#### 从旧包名 `@fishlikewater/dsh-mcp-manager` 升级

0.3.2 起本分支不再沿用上游名称，**必须重装一次**——profile 里记的是旧名的依赖键与 bundle 行，只改包内文件会出现「宿主按旧名装载、包内自称新名」的错配（客户端 bundle 的注册 id 校验失败，设置页会消失）：
...
- 插件优先用 `ctx.connection.fetch.register({ path: '/api/mcp/servers', methods: ['GET','POST'], requestBody: 'buffered', fetch })` 注册到 `/api` 共享通道的**精确 Fetch 路由**上。物理载体会先做 Host/Origin 信任栅栏与浏览器鉴权（`connection.requestRejection`），未鉴权的请求在到达插件之前就被 401/403 挡下。
...
### 0.3.4：按 DSH 0.1.6-alpha.1 的核对结果做的修复
...
0.3.0 针对 DSH 0.1.5 对齐的几处（都是按运行时安装的实际包逐条核对，不是猜的）：
...
- 设置页导航那一行用外壳默认图标：外壳按 section id 硬编码图标且没有第三方注册点，0.2.x 那套 CSS 覆盖依赖外壳类名哈希（`VOzbGW_*`），DSH 0.1.5 的外壳已不输出这些类名，遂删除。
...
- `@deepseek-ai/dsh-mcp-client` 以 peerDependency 声明，由运行时提供；`@deepseek-ai/schemastery` 与 `undici` 是普通依赖（后者只用于按 origin 定制 TLS dispatcher）。
- **`link:` 安装时注意"本地 dev 副本会盖住宿主副本"**：profile 用 `link:E:/work/ai/dsh-mcp-manager` 装本包时，Node 按真实路径解析依赖，插件 `import '@deepseek-ai/dsh-mcp-client'` 拿到的是**本仓库 `node_modules` 里的那份**（可能远旧于宿主实际加载的版本），而**不是**宿主 profile 里的那份。后果：本地跑得很正常，线上（正常的 npm 安装，peer 由宿主提供）才暴露版本差异——`test/log-contract.test.js` 就是用来盯这件事的：
  ```bash
