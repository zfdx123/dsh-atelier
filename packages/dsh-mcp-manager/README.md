...
- **不亮没有证据的绿**：`ctx.plugin()` 返回只说明实例建好了，**不说明连上了**——而 mcp-client 只在**失败**时打日志，连接成功不打任何日志。所以挂载完成后先显示「连接中」，只有拿到正向证据才转「已挂载」：工具注册（`mcp__<名>__<工具>` 出现，管理器给注入的 `tools` 服务包了一层只转发不改行为的 register 观察器）或成功日志（重连成功 / 重新同步工具）。确认窗口内（HTTP 默认 20 秒、stdio 5 秒）仍无任何证据就转「错误」并说明「服务器没有回应、也没有注册任何工具」。
  > **网络超时本来就有反馈，不用等确认窗口。** 实测黑洞地址（`192.0.2.1`，包被丢弃）：undici 的 connect timeout 是 **10 秒**，之后 mcp-client 自己就会打出 `connection attempt failed: … fetch failed ← UND_ERR_CONNECT_TIMEOUT Connect Timeout Error`，插件把它翻成带原因链与可操作提示的错误状态。所以确认窗口（20 秒）刻意设得**比它长**：那 10 秒里卡片显示「连接中」而不是绿，10 秒后显示的是这条**真实原因**，而不是窗口那句更含糊的「未确认」。窗口只兜住「连不上也不报错」的路径。
...
- **工具注册观察器会就地包装注入进来的 `tools` 服务**：包装只转发 `register`（原样返回 disposer）并在本插件 fiber 卸载时还原，但它是**对宿主服务对象的一次写操作**——若将来 `tools` 变成冻结对象或 Cordis 改用别的方式派发服务，这一步会装不上。装不上时不影响正确性：只损失「提前转绿」，确认窗口的超时判定照旧。
